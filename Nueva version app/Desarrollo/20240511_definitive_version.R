...
ui <- fluidPage(
  ...
  tags$head(
    ...
    .header-title {
      ...
      padding-right: 10px; /* Agrega un espacio entre el título y la descripción */
    }
    .header-description {
      margin: 0; /* Elimina el margen alrededor de la descripción */
      font-size: 16px; /* Reducir el tamaño de la fuente para la descripción */
    }
  ...
  headerPanel(
    div(
      ...
      div(
        class = "header-title",
        div("Consultas Data Warehouse Institucional")  # Título a la izquierda
      ),
      div(
        align = "right",
        img(src = "logo.png", height = "75px")  # Logo a la derecha
      )
    )
  ),
  p("Esta aplicación permite realizar consultas estandarizadas al Data Warehouse Institucional", style = "font-size: 20px;", class = "header-description")
  ,
    ...
    tabPanel("Órdenes de compra",
             sidebarLayout(
               sidebarPanel(
                 ...
                 downloadButton("downloadData_transacciones", "Descargar Detalle en Excel")
               ),
               mainPanel(
        ...
        tags$div(
          fluidRow(
            ...
            column(6,plotlyOutput("transacciones_total", height = "200px"))
            ,column(6,plotlyOutput("monto_transado", height = "200px"))
            ,column(6,plotlyOutput("oc_instituciones", height = "400px"))
            ,column(6,plotlyOutput("monto_instituciones",height = "400px"))
            ,column(6,plotlyOutput("oc_rubros",height = "400px"))
            ,column(6,plotlyOutput("monto_rubros",height = "400px"))
          )
          
        )
        ,tags$footer(
          style = "position: relative; bottom: 0; width: 100%; text-align: center;",
          p("© Desarrollado por División de Estudios y Políticas de Compra (2024)", style = "font-size: 15px;")
        )
    ...
    tabPanel("Usuarios OO.PP.",
             ...
             sidebarLayout(
               sidebarPanel(
                 ...
                 downloadButton("usr_downloadData", "Descargar Detalle en Excel")
               ),
               mainPanel(
                 tags$head(
                   tags$style(HTML("
    ...
    tabPanel("Reclamos a OO.PP.",
             ...
             sidebarLayout(
               sidebarPanel(
                 ...
                 downloadButton("rcl_downloadData", "Descargar Detalle en Excel")
               ),
               mainPanel(
                 tags$head(
                   tags$style(HTML("
...
# CONSULTAS PARA LISTAS DESPLEGABLES DATAWAREHOUSE================================================
    ...
    procedencias_disponibles <<- unique(dbGetQuery(con4, 
            ...
            CASE OC.porisintegrated 
              ...
              ELSE (CASE  OC.IDProcedenciaOC
                      ...
                      WHEN 701 THEN 'Licitacion Publica'
                      WHEN 1401 THEN 'Licitacion Publica'
                      WHEN 702 THEN 'Licitacion Privada'
                      ELSE 'Trato Directo' 
                    END)
            END  AS Procedencia 
  ...
  total_transacciones <- eventReactive(input$confirmar_btn, {
    ...
    if (!is.null(datos_consultados()) && nrow(datos_consultados()) > 0) {
        ...
        p_total_transacciones <- plot_ly(
          type = "indicator",
          value = total_oc$n_oc,
          mode = "number",
          title = list(text = "Total de Órdenes de Compra"),
        ...
        return(p_total_transacciones)
      }
    
  })
  
  # Crear una función reactiva para renderizar la cuadrícula de gráficos solo cuando se presione el botón
  total_monto <- eventReactive(input$confirmar_btn, {
    req(datos_crudos())
    ...
    if (!is.null(datos_crudos()) && nrow(datos_crudos()) > 0) {
      ...
      # Calcular el total de órdenes de compra para ese día
      monto_total <- datos_crudos() %>%
        distinct(CodigoOC, .keep_all = TRUE) %>%
        summarize(monto = sum(`Monto neto OC (pesos)`+`Impuesto OC (pesos)`,na.rm =TRUE))
      
      p_monto_total <- plot_ly(
        type = "indicator",
        value = monto_total$monto,
        mode = "number",
        title = list(text = "Monto Bruto Transado (pesos)"),
        domain = list(x = c(0, 1), y = c(0, 1)),
        height = 200
        ,number = list(
          prefix = "$"  # Prefijo para indicar dólares
          ,format = ".1fM" # Muestra millones con un decimal
        )
  ...
  total_institucion <- eventReactive(input$confirmar_btn, {
    req(datos_crudos())
  
  total_oc_por_institucion <- datos_crudos() %>% 
    group_by(`Nombre Institucion`) %>%
    summarise(n_oc = n_distinct(CodigoOC)) %>%
    arrange(desc(n_oc)) %>% 
    slice(1:10) # Mostrar solo el top 10
  
  # Crear la tabla con plot_ly
  tabla_ranking_institucion <- plot_ly(
    type = "table",
    name = list(text = "Top 10 Instituciones por cantidad de órdenes de compra"),
    header = list(
      values = c("Institución", "Total de Órdenes de Compra")
      ,line = list(width = 1, color = 'black')
      ,fill = list(color = c('grey','grey'))
      ,font = list(family = 'Verdana',size = 14, color = 'White')
      ),
    cells = list(
      values = list(total_oc_por_institucion$`Nombre Institucion`, total_oc_por_institucion$n_oc)
      ,line = list(width = 1, color = 'black')
      ,font = list(family = 'Verdana',size = 12, color = 'black')
      ,align = c('left', 'center')
      )
  )%>% layout(
    margin = list(t = 0, r = 0, b = 0, l = 0),
  ...
  return(tabla_ranking_institucion)
  })
  
  monto_institucion <- eventReactive(input$confirmar_btn, {
    req(datos_crudos())
    
    total_monto_por_institucion <- datos_crudos() %>% 
      group_by(`Nombre Institucion`) %>%
      distinct(CodigoOC,.keep_all = TRUE) %>% 
      summarise(monto = sum(`Monto neto OC (pesos)`+`Impuesto OC (pesos)`,na.rm =TRUE)) %>%
      arrange(desc(monto)) %>% 
      slice(1:10) # Mostrar solo el top 10
    
    # Crear la tabla con plot_ly
    monto_ranking_institucion <- plot_ly(
      type = "table",
      name = list(text = "Top 10 Instituciones por cantidad de órdenes de compra"),
      header = list(
        values = c("Institución", "Monto Bruto Transado")
        ,line = list(width = 1, color = 'black')
        ,fill = list(color = c('grey','grey'))
        ,font = list(family = 'Verdana',size = 14, color = 'White')
      ),
      cells = list(
        values = list(total_monto_por_institucion$`Nombre Institucion`, total_monto_por_institucion$monto)
        ,line = list(width = 1, color = 'black')
        ,font = list(family = 'Verdana',size = 12, color = 'black')
        ,align = c('left', 'center')
        )
    ...
    return(monto_ranking_institucion)
  })
  
  
  total_rubro <- eventReactive(input$confirmar_btn, {
    req(datos_crudos())
    
    if (input$detalle){
      total_oc_por_rubro <- datos_crudos() %>% 
        group_by(`Rubro Producto`) %>%
        summarise(n_oc = n_distinct(CodigoOC)) %>%
        arrange(desc(n_oc)) %>% 
        slice(1:10) # Mostrar solo el top 10
      
      # Crear la tabla con plot_ly
      tabla_ranking_rubro <- plot_ly(
        type = "table",
        name = list(text = "Top 10 Instituciones por cantidad de órdenes de compra"),
        header = list(
          values = c("Rubro", "Total de Órdenes de Compra")
          ,line = list(width = 1, color = 'black')
          ,fill = list(color = c('grey','grey'))
          ,font = list(family = 'Verdana',size = 14, color = 'White')
        ),
        cells = list(
          values = list(total_oc_por_rubro$`Rubro Producto`, total_oc_por_rubro$n_oc)
          ,line = list(width = 1, color = 'black')
          ,font = list(family = 'Verdana',size = 12, color = 'black')
          ,align = c('left', 'center')
        )
      )%>% layout(
        margin = list(t = 20, r = 0, b = 0, l = 0),
        xaxis = list(showticklabels = FALSE),
        yaxis = list(showticklabels = FALSE)
      )
      return(tabla_ranking_rubro)  
    } else {
      NULL
    }
  ...
  monto_rubro <- eventReactive(input$confirmar_btn, {
    req(datos_crudos())
    
    if (input$detalle){
      total_monto_por_rubro <- datos_crudos() %>% 
        group_by(`Rubro Producto`) %>%
        summarise(monto = sum(`Monto (neto) producto`,na.rm = TRUE)) %>%
        arrange(desc(monto)) %>% 
        slice(1:10) # Mostrar solo el top 10
      
      # Crear la tabla con plot_ly
      monto_ranking_rubro <- plot_ly(
        type = "table",
        name = list(text = "Top 10 Instituciones por cantidad de órdenes de compra"),
        header = list(
          values = c("Rubro", "Monto neto transado")
          ,line = list(width = 1, color = 'black')
          ,fill = list(color = c('grey','grey'))
          ,font = list(family = 'Verdana',size = 14, color = 'White')
        ),
        cells = list(
          values = list(total_monto_por_rubro$`Rubro Producto`, total_monto_por_rubro$monto)
          ,line = list(width = 1, color = 'black')
          ,font = list(family = 'Verdana',size = 12, color = 'black')
          ,align = c('left', 'center')
        )
      )%>% layout(
        margin = list(t = 30, r = 0, b =30, l = 0),
        xaxis = list(showticklabels = FALSE),
        yaxis = list(showticklabels = FALSE)
      )
      return(monto_ranking_rubro)  
    } else {
      NULL
    }
  ...
  output$transacciones_total <- renderPlotly({
  ...
  # Renderiza la cuadrícula de gráficos de transacciones
  output$monto_transado <- renderPlotly({
    req(input$consultar_btn)
    req(datos_consultados())
    total_monto()
  })
  ...
  # Renderiza la cuadrícula de gráficos de transacciones
  output$oc_instituciones <- renderPlotly({
    req(input$consultar_btn)
    req(datos_consultados())
    total_institucion()
  })
  ...
  # Renderiza la cuadrícula de gráficos de transacciones
  output$monto_instituciones <- renderPlotly({
    req(input$consultar_btn)
    req(datos_consultados())
    monto_institucion()
  })
  ...
  # Renderiza la cuadrícula de gráficos de transacciones
  output$monto_rubros <- renderPlotly({
    req(input$consultar_btn)
    req(datos_consultados())
    monto_rubro()
  })
    ...
    if (!is.null(compradores_consultados) && nrow(compradores_consultados()) > 0) {
      ...
      usr_aggregated <- compradores_consultados() %>%
        ...
        summarise(`Nombre completo` = `Nombre completo`[1]
                  ...
                  ,`Total de órdenes de compra` = n_distinct(CodigoOC)
                  ,`Monto total en pesos (millones)` = round(sum(`Monto Bruto CLP`, na.rm = TRUE)/1000000,1),
                  `Monto total en UF (millones)` = round(sum(`Monto Bruto CLF`, na.rm = TRUE)/1000000,2),
                  `Monto total en dólares (millones)` = round(sum(`Monto Bruto USD`, na.rm = TRUE)/1000000,2)) %>% 
        arrange(desc(`Total de órdenes de compra`))
